...
def scrape_domains(domains):
    ...
    for domain in domains.split("\n"):
        try:
         ...
         prompt = f"Based on the following information about the website {domain_name}:\n\nTitle: {page_title}\nDescription: {meta_description}\nMain Text: {main_text[:500]}...\n\nCraft a personalized cold email outreach for digital marketing purposes. The email should be friendly, engaging, and highlight how our digital marketing services can benefit the website's business. Keep the email concise and actionable, with a clear call-to-action.\n\nAdditionally, please include a signature with the following details:\n\nName: {st.session_state.user_info['name']}\nDigital Marketing Agency: {st.session_state.user_info['business_name']}\nWebsite: {st.session_state.user_info['website']}\nServices Offered: {st.session_state.user_info['business_description']}\nEmail: {st.session_state.user_info['email']}\nPhone Number: {st.session_state.user_info['phone_number']}"
            headers = {
            ...
            email_prompt = f"Here are the email addresses found on the website {domain_name}:\n\n{', '.join(emails)}\n\nBased on the website content and the personalized outreach email, which email address would be the most appropriate to send the outreach to for making a sale? Please make sure to only respond with the suggested email, nothing else!"
            email_data = {
                "model": "gpt-3.5-turbo",
